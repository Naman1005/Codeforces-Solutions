...
void click_clack_boom() {
    ...
    ip(n);
    string ans(n, '.');
    ll lo = 1, hi = n - 1, p = -1, q = -1;
    while (lo <= hi) {
        ll mid = (lo + hi) >> 1;
        vll a(mid + 1);
        iota(all(a), 0);
        (query(a) > 0) ? p = mid, hi = mid - 1 : lo = mid + 1;
    }
    if(p == -1)p = n-1, q = 0;
    else q = p, p--;
    vll rem;
    ans[p] = '(', ans[q] = ')';
    fro(i,0,n-1)if(ans[i] == '.')rem.push_back(i);
    while(!rem.empty()){
        if(rem.size() > 1){
            ll i = rem.back(); rem.pop_back();
            ll j = rem.back(); rem.pop_back();
            vll a{i,q,p,q,p,j,p,q};
            ll x = query(a);
            if(x == 10)ans[i] = '(', ans[j] = ')';
            else if(x == 4)ans[i] = '(', ans[j] = '(';
            else if(x == 6)ans[i] = ')', ans[j] = ')';
            else if(x == 2)ans[i] = ')', ans[j] = '(';
        } else {
            ll i = rem.back(); rem.pop_back();
            vll a{p, i};
            ll x = query(a);
