...
void click_clack_boom() {
    ...
    sort(all(a));
    for (auto &x : v) x = lower_bound(all(a), x) - a.begin() + 1;
    Fenwick fw(a.size());
    for (auto x : v) fw.update(x, 1);
    l = 0, r = n - 1;
    while (l < r) {
        if (v[l] == v[r]) {
            ++l, --r;
        ...
        lc = fw.query(v[l]), rc = fw.query(v[r]);
        if (lc < k && rc < k) {
            pr("NO");
            return;
        }
        (lc >= k && (rc < k || v[l] > v[r])) ? fw.update(v[l++], -1) : fw.update(v[r--], -1);
    }
    pr("YES");
}
int main() {
