...
ll fun(ll x, ll c, ll l, ll r) {
    ll i = __builtin_ctzll(c), a = x >> i, b = x & (c - 1), high = (prefXor(a + r) ^ prefXor(a + l - 1)) << i, low = ((r - l + 1) & 1) ? b : 0;
    return high ^ low;
}
void click_clack_boom() {
    ...
    ip(l, r, i, k);
    div = 1 << i;
    ll lo = l < k ? -(k - l) / div : (l - k + div - 1) / div, hi = r < k ? -((k - r + div - 1) / div) : (r - k) / div;
    pr(xorRange(l, r) ^ ((lo <= hi) ? fun(k, div, lo, hi) : 0LL), "");
}
int main() {
