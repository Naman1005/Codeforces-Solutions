...
void click_clack_boom() {
    ...
    if (up[n-2] == dwn[n-2] || dwn[n - 2] == dwn[n - 1] || up[n - 2] == up[n - 1]) {
        pr(n - 1, " ");
        return;
    }
    set<ll> upp = {up.back(), dwn.back(), dwn[n - 2]}, dwnn = {up.back(), dwn.back(), up[n - 2]};
    pre(i, n - 3, 0) {
        // pr(i,": ");
        // pr(upp, dwnn);
        if (up[i] == dwn[i] || upp.count(dwn[i]) || dwnn.count(up[i]) ) {
            pr(i + 1, " ");
            return;
        }
        upp.swap(dwnn);
