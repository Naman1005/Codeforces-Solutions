...
{
    ll n, k, curr = 0, x=-1, y=-1;
    ip(n, k);
    vll arr(n), prf(n + 1, 0), b(n - k + 1, -1);
    ip(arr);
    fro(i, 0, n - 1) prf[i + 1] = prf[i] + arr[i];
    pre(i, n - k, 0) b[i] = prf[i + k] - prf[i];
    multiset<ll, greater<ll>> ms;
    ms.insert(b.back());
    pre(i, n - 2 * k, 0) {
        ll mx = *ms.begin();
        if (mx + b[i] >= curr)
            x = b[i], y = mx, curr = mx + b[i];
        ms.insert(b[i + k - 1]);
    }
    ll ans1=-1, ans2 = -1;
    fro(i,0,n-k) {
