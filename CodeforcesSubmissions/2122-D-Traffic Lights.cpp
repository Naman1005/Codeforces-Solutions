...
void click_clack_boom() {
    ...
    // for time t, node u, if wait is w
    // we can go to time + 1, node u with w + 1
    // or we can go to time + 1, node v with w 
    //     where v is at oppostie end of (t % deg(u) + 1)th edge
    vll wait(n + 1, 1e18);
    wait[1] = 0;
    fro(t, 0, 1e18) {
        vll temp(n + 1, 1e18);
        fro(u, 1, n) if (wait[u] != 1e18) {
            temp[u] = min(temp[u], wait[u] + 1);
            ll idx = t % adj[u].size();
            temp[adj[u][idx]] = min(temp[adj[u][idx]], wait[u]);
        }
        wait = temp;
        if (wait[n] != 1e18) {
            pr(t + 1, wait[n], "");
