...
// degenerate triangle
// x + y > z, y + z > x, z + x > y
// here x: [a, b], y: [b, c], z: [c, d]
// only condition left is x + y > z (rest both are always satisfied)
// s = x + y
// s: [a+b, b+c]
// no of x,y pairs = no of x values for valid s (y can be found as s-x)
// a <= x <= b
// b <= y <= c  ->  b <= s-x <= c
// x <= s-b , x >= s-c
// therefore max(a, s-c) <= x <= min(b, s-b)
// no of x values (total pairs) = max(0, min(b, s-b) - max(a, s-c) + 1)
void click_clack_boom() {
    ll a, b, c, d, ans = 0;
    ip(a, b, c, d);
    vll f(b + c + 1, 0);
    fro(s, a + b, b + c) f[s] = max(0LL, min(b, s - b) - max(a, s - c) + 1);
    fro(s, a + b + 1, b + c) f[s] += f[s - 1];
    fro(z, c, d) if (z < b + c) ans += f[b + c] - f[z];
    pr(ans, " ");
}
int main() {
