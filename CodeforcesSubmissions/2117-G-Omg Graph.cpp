...
class DSUW {
    vll par, size, mn, mx;
public:
    DSUW(ll n) : par(n), size(n, 1), mn(n, LLONG_MAX), mx(n, LLONG_MIN) { iota(all(par), 0); }
    ...
    void unite(ll a, ll b, ll w) {
        ...
        mn[a] = min(min(mn[a], mn[b]), w), mx[a] = max(max(mx[a], mx[b]), w);
    }
    ll getMin(int x) {
        x = find(x);
        return mn[x];
...
void click_clack_boom() {
    ...
    for (auto [w, u, v] : ed) {
        ds.unite(u, v, w);
        if (ds.same_set(0, n - 1)) ans = min(ans, ds.getMin(0) + ds.getMax(0));
    }
    pr(ans, " ");
}
int main() {
