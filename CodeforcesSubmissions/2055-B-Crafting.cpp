...
{
    if constexpr (is_same<T, string>::value || is_same<typename decay<decltype(element)>::type, const char *>::value)
        cout << element << endl;
...
void solve()
{
    ...
    fro(i, 0, n - 1) if (a[i] >= b[i] && a[i] - b[i] < diff)
    {
        pr("NO");
        return;
    }
