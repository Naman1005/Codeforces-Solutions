...
void click_clack_boom() {
    ll n, m, k, ans = 0, mn = LLONG_MAX, temp;
    ip(n, m, k);
    vector<string> g(n);
    ip(g);
    vector<vll> prf(n + 1, vll(m + 1, 0));
    fro(i, 0, n - 1) fro(j, 0, m - 1) ans += (g[i][j] == 'g'), prf[i + 1][j + 1] = prf[i + 1][j] + prf[i][j + 1] - prf[i][j] + (g[i][j] == 'g');
    auto fun = [&](ll x1, ll y1, ll x2, ll y2) -> ll {
        if (x1 > x2 || y1 > y2) return 0;
        return prf[x2 + 1][y2 + 1] - prf[x1][y2 + 1] - prf[x2 + 1][y1] + prf[x1][y1];
    };
    temp = k - 1;
    fro(x, 0, n - 1) fro(y, 0, m - 1) if (g[x][y] == '.') mn = min(mn, fun(max(0LL, x - temp), max(0LL, y - temp), min(n - 1, x + temp), min(m - 1, y + temp)));
    pr(ans - mn, " ");
}
int main() {
