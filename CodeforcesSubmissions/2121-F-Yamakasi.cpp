...
ll fun(vll &v, ll tg) {
    ...
    for (auto x : v) {
        prf += x;
        if (cnt.find(prf - tg) != cnt.end()) ans += cnt[prf - tg];
        cnt[prf]++;
    }
    return ans;
}
void click_clack_boom() {
    ...
    auto fun2 = [&]() {
        ...
        ll tl = fun(temp, s), wx = 0;
        vll small;
        for (auto v : temp) (v == x) ? wx += fun(small, s), small.clear() : small.push_back(v);
        wx += fun(small, s), ans += (tl - wx), temp.clear();
    };
    fro(i, 0, n - 1)(a[i] > x) ? fun2() : temp.push_back(a[i]);
    fun2();
    pr(ans, " ");
}
int main() {
