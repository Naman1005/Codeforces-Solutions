...
void click_clack_boom() {
    ...
    vll a(n), b(m);
    ip(a, b);
    ll sa = accumulate(all(a), 0LL), sb = accumulate(all(b), 0LL), tot = sa * sb;
    map<ll, bool> mpa, mpb, exist;
    for (auto x : a) mpa[sa - x] = 1;
    for (auto x : b) mpb[sb - x] = 1;
    fro(x, 1, 2e5) {
        fro(y, 1, 2e5) {
            if (x * y > 2e5) break;
            if ((mpa[x] && mpb[y]) || (mpa[-x] && mpb[-y])) exist[x * y] = 1;
            if ((mpa[-x] && mpb[y]) || (mpa[x] && mpb[-y])) exist[-(x * y)] = 1;
        }
    }
    while (q--) {
