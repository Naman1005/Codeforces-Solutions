...
{
    ll n, k, mx = 0, low, high, ans, mid, res, i;
    string s;
    ip(n, k, s);
    vll a(n);
    ip(a);
    mx = *max_element(all(a)), low = 0, high = mx, ans = high;
    while (low <= high)
    {
        mid = (low + high) / 2, res =  i = 0;
        while (i < n)
        {
            bool f = 0;
            if (s[i] == 'R' && a[i] > mid)
            ...
            {
                if (s[i] == 'B' && a[i] > mid) f = 1;
                i++;
            }
            if (f) res++;
        }
        (res <= k) ? ans = mid, high = mid - 1 : low = mid + 1;
    }
    pr(ans, " ");
}
int main()
