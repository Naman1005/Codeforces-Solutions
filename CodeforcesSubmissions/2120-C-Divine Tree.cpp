...
void click_clack_boom() {
    ...
    vll up(n + 1), down(n + 1);
    vb vis(n + 1, 0);
    fro(i, 1, n) up[i] = (i < n ? i + 1 : 0), down[i] = (i > 1 ? i - 1 : 0);
    vpl ans;
    pre(i, n, 2) {
        if (nd <= 0) break;
        ll u = up[i], v = down[i];
        if (u) vis[min(i, u)] = 1, down[u] = v;
        if (v) vis[min(i, v)] = 1, up[v] = u;
        if (i == rt) rt = v;
        up[i] = down[i] = 0;
        if (nd >= i - 1)
            ans.push_back({i, 1}), nd -= i - 1;
        else
            ans.push_back({i, i - nd}), nd = 0;
    }
    pr(rt, " ");
    fro(k, 1, n - 1) if (!vis[k]) pr(k, k + 1, " ");
    pr(ans);
}
int main() {
