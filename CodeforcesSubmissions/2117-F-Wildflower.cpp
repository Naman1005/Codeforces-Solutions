...
void click_clack_boom() {
    ...
    while (!q.empty()) {
        ...
        for (auto v : adj[u]) {
            if (v == par[u]) continue;
            if (f2 && div == -1) div = u;
            f2 = 1, f = 0, par[v] = u, dep[v] = dep[u] + 1;
            q.push(v);
        }
        if (f) leaves.push_back(u);
    }
    if (leaves.size() > 2) {
        pr("0");
        return;
    ...
    // pr(div+1);
    // fro(i, 0, leaves.size() - 1) pr(leaves[i] + 1);
    // pr(" ");
    mint ans;
    if (leaves.size() == 1) {
        ans = mint(2).power(n);
    ...
    ans = mint(2).power(dep[div]);
    ll d1 = dep[leaves[0]], d2 = dep[leaves[1]];
    // pr(dep[div],d1, d2,"**");
    ans *= (d1 == d2 ? mint(2) : (mint(2).power(abs(d1 - d2)) + mint(2).power(abs(d1 - d2) - 1)));
    pr(ans.val(), " ");
}
int main() {
