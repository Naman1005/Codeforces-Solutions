...
struct _ {
    _(ll n) {
        prime.resize(n + 1, 1);
        prime[0] = prime[1] = 0;
        fro(i, 2, n) if (prime[i] && i * i <= n) for (ll j = i * i; j <= n; j += i) prime[j] = 0;
    }
} __(1e5);
// let p be the samllest prime which is not a factor of k
// we want c1, c2, ..., cn such that ai + ci*k = 0 mod p
// => ci = (-ai/k) mod p
// => ci = (-ai * k ^ -1) mod p
