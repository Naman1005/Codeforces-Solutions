...
void click_clack_boom()
{
    ...
    vll a(n);
    ip(a);
    map<ll, ll> mx1, mx2, lst;
    fro(i, 0, n - 1)
    {
        ll dif = i - (lst.count(a[i] - 1) ? lst[a[i] - 1] : -1);
        if (dif > mx1[a[i] - 1])
            mx2[a[i] - 1] = mx1[a[i] - 1], mx1[a[i] - 1] = dif;
        else if (dif > mx2[a[i] - 1])
            mx2[a[i] - 1] = dif;
        lst[a[i] - 1] = i;
    }
    fro(i, 0, k - 1)
    {
        ll dif = n - lst[i];
        if (dif > mx1[i])
            mx2[i] = mx1[i], mx1[i] = dif;
        else if (dif > mx2[i])
            mx2[i] = dif;
    }
    // pr(mx1, mx2);
    fro(i, 0, k - 1) ans = min(ans, max((mx1[i] + 1) / 2, mx2[i]));
    pr(ans - 1, " ");
}
int main()
{
