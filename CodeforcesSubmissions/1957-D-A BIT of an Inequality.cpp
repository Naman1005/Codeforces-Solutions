...
void click_clack_boom() {
    ...
    a.insert(a.begin(), 0);
    vll prf(n + 1, 0);
    fro(i, 1, n) prf[i] = prf[i - 1] ^ a[i];
    // pr(prf);
    vector<vll> ps(m, vll(n + 2, 0));
    fro(i, 0, m - 1) {
        fro(j, 0, n) { ps[i][j + 1] = ps[i][j] + (((prf[j] >> i) & 1) ? 1 : 0); }
    }
    fro(i, 1, n) {
        ll h = 0;
        fro(b, 0, m - 1) if ((a[i] >> b) & 1) h = b;
        ll l1 = ps[h][i], l0 = i - l1, r1 = ps[h][n + 1] - ps[h][i], r0 = (n - i + 1) - r1;
        ans += (l1 * r1) + (l0 * r0);
    }
    // pr(ps);
    pr(ans, "");
