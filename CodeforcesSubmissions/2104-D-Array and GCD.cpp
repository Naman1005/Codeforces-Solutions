...
void fun(ll n) {
    ...
    ___.resize(n + 1, true);
    ___[0] = ___[1] = false;
    fro(i, 2, n) if (___[i] && i * i <= n) for (ll j = i * i; j <= n; j += i) ___[j] = false;
    fro(i, 2, n) if (___[i]) prime.push_back(i);
    prf.resize(prime.size());
    prf[0] = prime[0];
    fro(i, 1, prime.size() - 1) prf[i] = prf[i - 1] + prime[i];
}
