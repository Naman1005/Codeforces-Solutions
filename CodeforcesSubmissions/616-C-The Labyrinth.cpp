...
class DSU {
 ...
 void click_clack_boom() {
    ...
    vector<string> a(n);
    ip(a);
    auto flat = [&](ll i, ll j) -> ll {
        return m * i + j;
    };
    fro(i, 0, n - 1) {
        fro(j, 0, m - 1) {
            if (a[i][j] == '*') continue;
            if (i > 0 && a[i - 1][j] != '*') d.unite(flat(i, j), flat(i - 1, j));
            if (j > 0 && a[i][j - 1] != '*') d.unite(flat(i, j), flat(i, j - 1));
            if (i < n - 1 && a[i + 1][j] != '*') d.unite(flat(i, j), flat(i + 1, j));
            if (j < m - 1 && a[i][j + 1] != '*') d.unite(flat(i, j), flat(i, j + 1));
        }
    }
    set<ll> cnt;
    // pr(cnt.size(), " ");
    vector<string> ans = a;
    fro(i, 0, n - 1) {
        fro(j, 0, m - 1) if (a[i][j] == '*') {
            set<ll> st;
            if (i > 0 && a[i - 1][j] != '*') st.insert(d.find(flat(i - 1, j)));
            if (j > 0 && a[i][j - 1] != '*') st.insert(d.find(flat(i, j - 1)));
            if (i < n - 1 && a[i + 1][j] != '*') st.insert(d.find(flat(i + 1, j)));
            if (j < m - 1 && a[i][j + 1] != '*') st.insert(d.find(flat(i, j + 1)));
            ll sum = 1;
            for (auto it : st) { sum += d.get_size(it); }
            ans[i][j] = '0' + (sum % 10);
        }
    }
    pr(ans);
