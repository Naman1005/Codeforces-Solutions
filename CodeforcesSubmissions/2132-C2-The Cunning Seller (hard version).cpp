...
vll pw(25, 1);
struct _ {
    _() { fro(i, 1, 24) pw[i] = pw[i - 1] * 3; }
} __;
void click_clack_boom() {
    ll n, k, nn, mn = 0, done = 0, ex = 0;
    ip(n, k);
    nn = n;
    if (k >= n) {
        pr(n * 3, "");
    ...
    cnt[0] = n;
    fro(i, 0, 24) {
        if (done >= (n - k + 1) / 2) break;
        ll curr = min(cnt[i] / 3, ((n - k + 1) / 2) - done);
        cnt[i] -= curr * 3, cnt[i + 1] += curr;
        ex += curr * pw[i], done += curr;
    }
    pr(n * 3 + ex, "");
}
int main() {
