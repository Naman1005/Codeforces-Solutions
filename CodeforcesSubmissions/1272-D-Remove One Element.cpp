...
void click_clack_boom()
{
    ll n, ans = 1;
    ip(n);
    vll a(n), ea(n, 1), ba(n, 1);
    ip(a);
    fro(i, 1, n - 1) if (a[i] > a[i - 1]) ea[i] += ea[i - 1], ans = max(ans, ea[i]);
    pre(i, n - 2, 0) if (a[i] < a[i + 1]) ba[i] += ba[i + 1];
    fro(i, 1, n - 2) if (a[i - 1] < a[i + 1]) ans = max(ans, ea[i - 1] + ba[i + 1]);
    pr(ans);
}
int main()
{
