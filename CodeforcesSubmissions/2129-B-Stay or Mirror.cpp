...
void click_clack_boom() {
    ll n, idx = -1, ans = 0, x, y;
    ip(n);
    vll a(n), b;
    ip(a);
    b = a;
    fro(i, 0, n - 1) if (a[i] == n) idx = i;
    fro(k, 0, n - 1) if (k != idx) {
        x = y = 0;
        fro(j, k + 1, n - 1) x += (j != idx && a[k] < a[j]);
        fro(i, 0, k - 1) y += (i != idx && a[i] > a[k]);
        x += (k < idx), y += (idx < k);
        if (x < y) b[k] = 2 * n - a[k];
    }
    fro(i, 0, n - 1) fro(j, i + 1, n - 1) ans += (b[i] > b[j]);
    pr(ans, "");
}
int main() {
