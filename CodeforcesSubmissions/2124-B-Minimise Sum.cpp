...
void click_clack_boom() {
    ...
    vll a(n);
    ip(a);
    vll pm(n), bad(n, 1e18), prf(n, 1e18), suf(n + 1, 0);
    ans = pm[0] = a[0];
    fro(i, 1, n - 1) pm[i] = min(pm[i - 1], a[i]), ans += pm[i];
    fro(i, 1, n - 1)(a[i] < pm[i - 1]) ? bad[i] = pm[i - 1] - a[i] : bad[i] = 0;
    fro(i, 1, n - 1) prf[i] = min(prf[i - 1], bad[i]);
    pre(i, n - 1, 0) suf[i] = suf[i + 1] + pm[i];
    fro(i, 1, n - 1) if (suf[i] > neg + min(prf[i - 1], a[i])) neg = suf[i] - min(prf[i - 1], a[i]);
    pr(ans - neg, " ");
}
int main() {
