...
void click_clack_boom() {
    ...
    ip(a);
    for (auto &v : a) for (auto &x : v) mx = max(mx, x);
    auto fun = [&](ll x) -> bool {
        fro(i, 0, n - 1) fro(j, 0, m - 1) if (a[i][j] > x + 1) return 0;
        vector<pll> v;
        fro(i, 0, n - 1) fro(j, 0, m - 1) if (a[i][j] == x + 1) v.push_back({i, j});
        if (v.empty()) return 1;
        auto [rr, cc] = v[0];
        ll c = -1, r = -1;
        bool fl = 0;
    ...
    ll lo = 0, hi = mx, ans = mx, mid;
    while (lo <= hi) {
        mid = (lo + hi) >> 1;
        (fun(mid)) ? ans = mid, hi = mid - 1 : lo = mid + 1;
    }
    pr(ans, " ");
}
int main() {
