...
struct is_container<T, void_t<decltype(std::declval<T>().begin()),
                              decltype(std::declval<T>().end())>> : true_type {
};
template <typename T1, typename T2> void pr(const pair<T1, T2> &p) {
...
void click_clack_boom() {
    ...
    function<bool(ll)> check = [&](ll x) {
        ...
        fro(i, 0, n - 1) {
            ...
            if (cnt == x)
                cnt = 0, num++, fill(all(seen), 0);
        }
        return num >= k;
    };
    ll lo = 1, hi = mx, ans = 1, mid;
    while (lo <= hi) {
        mid = (lo + hi) / 2;
        if (check(mid))
            lo = mid + 1, ans = mid;
...
int main() {
    ...
    cin.tie(NULL); cout.tie(NULL); inp_out();
