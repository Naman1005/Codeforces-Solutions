...
ll prefixOR(ll m)
{
    if (m <= 0)
        return 0;
    ll L = m - 1, hb = hi_bit(L);
    return (1 << (hb + 1)) - 1;
}
void click_clack_boom()
{
    ll n, x, f = -1, M, cd, temp, br, rem;
    ip(n, x);
    fro(y, 0, n) if ((y & (~x)) != 0)
    {
        f = y;
    ...
    if (f == -1)
        f = n + 1;
    cd = min(n, f);
    if (cd < n || prefixOR(cd) == x)
        M = cd;
    else
        M = cd - 1;
    vector<ll> ans;
    fro(i, 0, M - 1) ans.push_back(i);
    temp = M, br = prefixOR(M), rem = n - temp;
    if (rem > 0 && br != x)
        ans.push_back(x), temp++, rem--, br |= x;
    while (temp < n)
