...
void click_clack_boom() {
    ...
    vll a(n);
    ip(a);
    vll freq(n + 1, 0), rvec, cvec;
    for (auto x : a) freq[x]++;
    vector<bool> rem(n + 1, 0), curr(n + 1, 0);
    fro(i, 0, n - 2) {
        freq[a[i]]--;
        if (rem[a[i]]) rem[a[i]] = 0, temp--;
        if (!curr[a[i]]) curr[a[i]] = 1, cvec.push_back(a[i]);
        if (!temp) {
            bool f = 1;
            for (auto v : cvec)
                if (freq[v] == 0) {
            ...
            if (f) {
                ans++;
                for (auto v : rvec) rem[v] = 0;
                rvec.clear();
                for (auto v : cvec) rem[v] = 1, rvec.push_back(v);
                temp = rvec.size();
                for (auto v : cvec) curr[v] = 0;
                cvec.clear();
            }
        }
    }
