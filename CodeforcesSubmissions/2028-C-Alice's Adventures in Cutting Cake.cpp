...
void click_clack_boom()
{
    ll n, m, v, ans = -1;
    ip(n, m, v);
    vll a(n), pref(n + 1, 0), bef(m + 1), aft(m + 1);
    ip(a);
    fro(i, 0, n - 1) pref[i + 1] = pref[i] + a[i];
    ll j = 0;
    fro(i, 1, m)
    {
        while (j <= n && pref[j] - pref[bef[i - 1]] < v)
            j++;
        bef[i] = j;
    }
    aft[0] = j = n;
    fro(i, 1, m)
    {
        while (j >= 0 && pref[aft[i - 1]] - pref[j] < v)
            j--;
        aft[i] = j;
    }
    fro(i, 0, m) if (bef[i] <= aft[m - i]) ans = max(ans, pref[aft[m - i]] - pref[bef[i]]);
    cout << ans << "\n";
}
int main()
{
