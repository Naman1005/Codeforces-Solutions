...
void click_clack_boom()
{
    ll n, m, idx;
    ip(n, m);
    vector<vll> gr(n, vll(m));
    ip(gr);
    vll sm(n, 0), ans;
    fro(i, 0, n - 1) for (auto x : gr[i]) sm[i] += x;
    priority_queue<pair<ll, ll>> pq;
    fro(i, 0, n - 1) pq.push({sm[i], i});
    while (!pq.empty())
    {
        idx = pq.top().second, pq.pop();
        fro(j, 0, m - 1) ans.push_back(gr[idx][j]);
    }
    fro(i, 1, ans.size() - 1) ans[i] += ans[i - 1];
    pr(accumulate(ans.begin(), ans.end(), 0LL), " ");
}
int main()
{
