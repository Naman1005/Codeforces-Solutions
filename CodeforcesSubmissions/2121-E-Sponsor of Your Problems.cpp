...
void click_clack_boom() {
    string l, r;
    ip(l, r);
    ll n = l.size(), ans = 1e18;
    vector<array<array<ll, 2>, 2>> dp(n + 1);
    fro(i, 0, n) fro(a, 0, 1) fro(b, 0, 1) dp[i][a][b] = 1e18;
    dp[0][0][0] = 0;
    fro(i, 0, n - 1) fro(tl, 0, 1) fro(tr, 0, 1) {
        ll temp = dp[i][tl][tr], lo, hi;
        if (temp == 1e18) continue;
        lo = tl ? 0 : (l[i] - '0'), hi = tr ? 9 : (r[i] - '0');
        fro(d, lo, hi) {
            ll ntL = tl | (d > (l[i] - '0')), ntR = tr | (d < (r[i] - '0'));
            dp[i + 1][ntL][ntR] = min(dp[i + 1][ntL][ntR], temp + (d == (l[i] - '0')) + (d == (r[i] - '0')));
        }
    }
    fro(a, 0, 1) fro(b, 0, 1) ans = min(ans, dp[n][a][b]);
    pr(ans, " ");
}
int main() {
