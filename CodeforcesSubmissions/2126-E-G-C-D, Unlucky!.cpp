...
void click_clack_boom() {
    ll n, g;
    ip(n);
    vll p(n), s(n), a(n), np(n), ns(n);
    ip(p, s);
    fro(i, 1, n - 1) if (p[i - 1] % p[i]) {
        pr("NO");
        return;
    }
    pre(i, n - 1, 1) if (s[i] % s[i - 1]) {
        pr("NO");
        return;
    }
    fro(i, 0, n - 1) a[i] = p[i] / (__gcd(p[i], s[i])) * s[i];
    np[0] = a[0];
    fro(i, 1, n - 1) np[i] = __gcd(np[i - 1], a[i]);
    ns.back() = a.back();
    pre(i, n - 2, 0) ns[i] = __gcd(ns[i + 1], a[i]);
    pr((np == p && ns == s) ? "YES" : "NO");
}
int main() {
