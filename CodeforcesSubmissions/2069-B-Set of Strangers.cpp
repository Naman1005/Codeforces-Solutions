...
void click_clack_boom()
{
    ...
    vll ans(n * m + 1, 0);
    fro(i, 0, n - 1) fro(j, 0, m - 1) ans[a[i][j]] = 1;
    fro(i, 0, n - 1) fro(j, 0, m - 1) fro(k, 0, 3) if (i + dr[k] >= 0 && i + dr[k] < n && j + dc[k] >= 0 && j + dc[k] < m && a[i + dr[k]][j + dc[k]] == a[i][j]) ans[a[i][j]] = 2;
    pr(accumulate(all(ans), 0LL) - *max_element(all(ans)), " ");
}
int main()
{
