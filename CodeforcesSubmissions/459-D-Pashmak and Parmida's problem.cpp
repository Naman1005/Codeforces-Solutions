...
private:
    ...
    ll n;
 public:
    Fenwick(ll size) : n(size) { fen.assign(n + 1, 0); }
    void update(ll i, ll val) {
        for (ll idx = i + 1; idx <= n; idx += idx & -idx) fen[idx] += val;
    }
    void overwrite(ll i, ll val) {
        ll current = get(i, i);
        ll diff = val - current;
        update(i, diff);
    }
    ll query(ll i) {
        ll sum = 0;
        for (ll idx = i + 1; idx > 0; idx -= idx & -idx) { sum += fen[idx]; }
        return sum;
    }
    ll get(ll l, ll r) { return query(r) - (l > 0 ? query(l - 1) : 0); }
};
void click_clack_boom() {
    ...
    map<ll, ll> mp;
    Fenwick fw(1e6);
    fro(i, 0, n - 1) left[i] = ++mp[a[i]];
    mp.clear();
    pre(i, n - 1, 0) right[i] = ++mp[a[i]];
    // pr(left, right);
    pre(i, n - 1, 0) {
        ans += fw.query(left[i] - 1);
        fw.update(right[i], 1);
    }
    pr(ans, "");
}
